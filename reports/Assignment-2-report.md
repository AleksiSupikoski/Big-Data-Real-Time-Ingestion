...
Service profile is defined in two .xml files for input and ouptut of the clientBatchIngestApp, One limits the number of files (per hour) and file extension that can be accepted, the other sets a constraint on size of data per hour that is allowed for ingestion. For testing for each tenant (two tenants) a personal batch ingestion service profile has been developed, the first one limits user to stage 500 .csv files for ingestion per-hour and 100 Gigabytes per-hour.
